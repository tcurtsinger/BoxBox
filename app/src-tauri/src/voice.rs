...
fn worker(dir: PathBuf, jobs: Receiver<Job>, current: Arc<Mutex<Option<Arc<rodio::Sink>>>>) {
    ...
    for job in jobs {
        let result = (|| -> Result<(), String> {
            ...
            let sink = Arc::new(rodio::Sink::try_new(&handle).map_err(|e| format!("sink: {e}"))?);
            sink.set_volume(job.volume.clamp(0.0, 1.0));
            sink.append(source);
            *current.lock().unwrap_or_else(|p| p.into_inner()) = Some(sink.clone());
            // Blocks until the line finishes — or returns early if voice_cancel
            // stopped the sink from its own thread (the lock is NOT held here).
...
impl VoiceState {
    pub fn new(resource_dir: PathBuf) -> Self {
        ...
        std::thread::Builder::new()
            .name("voice".into())
            .spawn(move || worker(resource_dir, rx, cur))
            .expect("spawn voice worker");
        Self { jobs: tx, current }
    }
}
...
/// Stop the line being spoken (safety-callout pre-emption). The blocked
/// `voice_speak` returns immediately.
#[tauri::command]
...
mod tests {
    ...
    fn piper_speaks() {
        ...
        let cur = current.clone();
        let t = std::thread::spawn(move || worker(dir, rx, cur));
        let (done_tx, done_rx) = sync_channel(1);
        tx.send(Job {
